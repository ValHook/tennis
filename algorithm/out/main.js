...
import { MATCH_DURATION_MINUTES, MAX_SIMULATIONS_PER_NODE } from "./constants.js";
...
function SessionFromInput() {
    ...
    for (let i = 0; i < n_courts; ++i) {
        courts.push({
            id: i + 1,
            availability_minutes: PromptInt("Court #" + (i + 1) + " minutes?", MATCH_DURATION_MINUTES)
        });
    }
    ...
    for (let i = 0; i < n_players; ++i) {
        const player = {
            ...
            availability_minutes: PromptInt("Player #" + (i + 1) + " minutes?", min_minutes, max_minutes, allowed_durations)
        };
...
function FixturesFromStage(stage, players, seed) {
    ...
    const pairwise_single_probability = Math.pow(single_probability, 2) / (Math.abs(n_single_courts * 2 - 1));
    const pairwise_double_probability = Math.pow(double_probability, 2) / (Math.abs(n_double_courts * 4 - 1));
