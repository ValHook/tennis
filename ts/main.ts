...
function SessionFromInput(): Session {
    ...
    for (let i = 0; i < n_courts; ++i) {
        courts.push({
            ...
            availability_minutes: 
                PromptInt("inputCourtDuration" + i, "Court #" + (i+1) + " availability in minutes?", match_duration_minutes)
        });
    }
    ...
    for (let i = 0; i < n_players; ++i) {
        const player = {
            ...
            availability_minutes: PromptInt("inputPlayerMinutes" + i, "Player #" + (i+1) + " availability in minutes?", 
...
function main() {
    document.querySelector<HTMLInputElement>("#inputCourtCount")?.addEventListener("change", (event) => {
      changeCourtCount(parseInt((event?.target as HTMLInputElement)?.value));
    });
    document.querySelector<HTMLInputElement>("#inputPlayerCount")?.addEventListener("change", (event) => {
    ...
    document.querySelector<HTMLInputElement>("#generate")?.addEventListener("click", (event) => {
        tennisGen(SessionFromInput());
    });
    document.querySelector<HTMLInputElement>("#regenerate")?.addEventListener("click", (event) => {
        tennisGen(session || SessionFromInput());
    });
    document.querySelector<HTMLInputElement>("#clipboard")?.addEventListener("click", (event) => {
