...
import { MAX_DELTAS_PER_HAPPENING_TYPE, MAX_SIMULATIONS_PER_NODE, NUM_PLAYERS_DOUBLE, NUM_PLAYERS_SINGLE, } from "./constants.js";
import { Status, StatusOr } from "./status.js";
...
export function ComputeRoster(session, seed) {
    ...
    for (let i = 0; i < session.stages.length; ++i) {
        ...
        if (!stage_rotations.ok()) {
            return {
                rotations: StatusOr.Error("Problematic stage: " +
                    i +
                    ". Most common error: " +
...
function ComputeStageRotations(session, stage_id, rotation_id, checker, rng) {
    const stage = session.stages[stage_id];
    if (rotation_id >= stage.n_rotations) {
        return StatusOr.Ok([]);
    }
    const rejections = {};
    const downstream_rejections = {};
    for (let i = 0; i < MAX_SIMULATIONS_PER_NODE; ++i) {
        ...
        for (const single of rotation.singles) {
            ...
            checker_with_proposal.add(Happening.PlayingSingleAgainst(single.player_a, single.player_b));
        }
        for (const double of rotation.doubles) {
            ...
            checker_with_proposal.add(Happening.PlayingDoubleAgainst(double.player_a1, double.player_b1));
            checker_with_proposal.add(Happening.PlayingDoubleAgainst(double.player_a1, double.player_b2));
            checker_with_proposal.add(Happening.PlayingDoubleAgainst(double.player_a2, double.player_b1));
            checker_with_proposal.add(Happening.PlayingDoubleAgainst(double.player_a2, double.player_b2));
            checker_with_proposal.add(Happening.PlayingDoubleWith(double.player_a1, double.player_a2));
            checker_with_proposal.add(Happening.PlayingDoubleWith(double.player_b1, double.player_b2));
        }
        ...
        // Recurse.
        const next_rotations = ComputeStageRotations(session, stage_id, rotation_id + 1, checker_with_proposal, rng);
    ...
    const could_advance_downstream = Object.keys(downstream_rejections).length > 0;
    const most_representative_rejection_pool = could_advance_downstream
        ? downstream_rejections
        : rejections;
    const most_representative_rejection = Object.entries(most_representative_rejection_pool).reduce((best, next) => {
        if (!best) {
            return next;
        }
        return next[1] > best[1] ? next : best;
    });
    return StatusOr.Error(could_advance_downstream
...
export class HappeningDeltaChecker {
    ...
    copy() {
        ...
        Object.keys(this.happenings).forEach((type) => {
            ...
            out.happenings[casted_type] = this.recordCopy(this.happenings[casted_type]);
        });
        return out;
    }
    add(happening) {
    ...
    validate() {
        for (const [type, record] of Object.entries(this.happenings)) {
            const casted_type = type;
            const min = Object.keys(record).length < this.permutations[casted_type]
                ? 0
